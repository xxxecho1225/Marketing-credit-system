...
function MarkingUpload() {
  ...
  const [ipfsHashMerchant, setIfpsHashMerchant] = useState("");
  console.log("商户地址1:", merchantAddress);
  console.log("商户名:", username);
  console.log("上传文件的哈希11:", ipfsHashMerchant);
  ...
  const onFinish = (values) => {
    const {  productname,description,promotions, } = values;
    // 处理表单提交的值
    console.log("产品名字:", productname);
    console.log("产品描述:", description);
    console.log("推广活动名:", promotions);

    handleUploadMarking({ username, merchantAddress,...values, ipfsHashMerchant });
  };

  const handleUploadMarking = async (info) => {
    try {
      ...
      formData.append("promotions", info.promotions);
      formData.append("ipfsHashMerchant", info.ipfsHashMerchant);

      // Make a POST request to the backend
      const response = await fetch("http://localhost:8080/api/marking/upload", {
        method: "POST",
        body: formData,
      });
      ...
      if (result.success) {
        ...
        message.success("上传营销信息成功");
      } else {
        message.error(`上传营销信息失败: ${result.error}`);
      }
    } catch (error) {
  ...
  const props = {
    name: "file",
    multiple: true,
    action: "http://localhost:8080/api/marking/fileupload",
    headers: {
    ...
    onChange(info) {
      ...
      if (status === "done") {
        ...
        if (response && response.success) {
          ...
          // 保存 IPFS 哈希值
          setIfpsHashMerchant(response.filePath);
  ...
  return (
    ...
    >
      ...
      <Dragger {...props}>
        ...
        <p className="ant-upload-text">点击此区域上传营销信息</p>
