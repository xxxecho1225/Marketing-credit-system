...
// Handle POST request to uploadMarketingInfo
router.post("/upload",upload.single("file"), async (req, res) => {
  try {
    const {
     ...
     // 确保所有参数都存在
     if (!username || !merchantAddress || !productname || !description || !promotions || !ipfsHashMerchant) {
      return res.status(400).json({ success: false, message: '缺少必要的参数' });
    }

    // Call the uploadCreditCredentials function
    const tx = await contract.functions.uploadMarketingInfo(
      username,
      merchantAddress,
      productname,
      description,
...
router.get("/list", async (req, res) => {
  try {
    ...
    const allMarkingsDetails = await contract.getAllMarketingInfo();

    const processedMarkingsDetails = allMarkingsDetails.map(detail => ({
    ...
    console.log("所有营销信息的详细信息:", processedMarkingsDetails);

    // 将 allCreditDetails 返回给前端
    res.json({ success: true, processedMarkingsDetails });
  } catch (error) {
    console.error(error);
